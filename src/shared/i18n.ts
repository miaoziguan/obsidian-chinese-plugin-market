...
export const STRINGS = {
	...
	"updates.pin": { zh: "选择版本" },
	"version.title": { zh: "选择版本 · {name}" },
	"version.latest": { zh: "保持最新（自动更新）" },
	"version.latest.hint": { zh: "跟随仓库最新版本，检测到新版本时提示更新" },
	"version.loading": { zh: "正在获取版本列表…" },
	"version.empty": { zh: "未获取到可用版本" },
	"version.empty.hint": { zh: "该仓库可能没有发布 Release 或 Tag" },
	"version.fail": { zh: "获取版本列表失败：{msg}" },
	"version.pinned": { zh: "已固定 v{version}" },
	"version.current": { zh: "当前" },
	"version.pin.done": { zh: "已将 {name} 固定到 v{version}" },
	"version.unpin.done": { zh: "已将 {name} 改为保持最新" },
	"version.refresh": { zh: "刷新" },
	...
	"journal.col.uninstalled": { zh: "最后卸载" },
	"journal.col.note": { zh: "备注" },
	"journal.estimated": { zh: "估算值：该插件在足迹开始记录之前就已安装，时间取自插件目录的文件时间" },
	// 插件依赖提示与关联（dep）
	"dep.section": { zh: "依赖" },
	"dep.required": { zh: "必需" },
	"dep.optional": { zh: "可选" },
	...
	"view.tab.favorites": { zh: "收藏" },
	"fav.tab.hint": { zh: "分组管理已收藏的插件：建组、换组、取消收藏、直达详情。" },
	"fav.filter.keyword.ph": { zh: "搜索已收藏插件…" },
	"fav.filter.group.all": { zh: "全部分组" },
	"fav.group.none": { zh: "未分组" },
	"fav.group.new": { zh: "新建分组" },
	"fav.group.new.ph": { zh: "输入分组名称" },
	"fav.group.rename": { zh: "重命名" },
	"fav.group.rename.ph": { zh: "输入新的分组名称" },
	"fav.group.delete": { zh: "删除分组" },
	"fav.group.delete.confirm": { zh: "删除分组「{name}」？组内插件将回到未分组（不会取消收藏）。" },
	"fav.move.ph": { zh: "移动到分组" },
	"fav.open.detail": { zh: "打开插件详情" },
	"fav.remove": { zh: "取消收藏" },
	"fav.count": { zh: "找到 {shown} / 共 {total}" },
	"fav.empty": { zh: "暂无收藏。在浏览页签点卡片上的星标即可收藏。" },
	"fav.empty.filtered": { zh: "当前筛选条件下没有收藏的插件。" },
	"css.new": { zh: "新建片段" },
