...
import { writeTMNote, removeTMNote, TM_FOLDER, parseTMNote, type TMEntry } from "./translation-memory";
...
export default class ChinesePluginMarketPlugin extends Plugin {
	...
	private async loadLegacyVectorIndex(): Promise<import("./embedding").VectorIndex | null> {
		const adapter = this.app.vault.adapter;
		try {
			if (await adapter.exists(this.legacyVectorBinFilePath)) {
				const buf = await adapter.readBinary(this.legacyVectorBinFilePath);
				const { decodeVectorIndex } = await import("./vec-codec");
